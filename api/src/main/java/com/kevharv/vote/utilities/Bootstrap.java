...
public class Bootstrap {
    ...
    public CommandLineRunner bootstrapDataset(
        ...
        return (args) -> {
            ...
            positions.add(
                    new Position("Mayor", "Mayor of Dallas", geographyRepository.findById(Long.parseLong("4")).get()));
            positionRepository.saveAll(positions);

            // Create Politicians
            log.info("Adding politicians");
            List<Politician> politicians = new ArrayList<Politician>(8);
            politicians.add(new Politician("Barack", "Obama", partyRepository.getPartyByName("Democratic Party"),
                    geographyRepository.findById(geographies.get(0).getId()).get()));
            politicians.add(new Politician("Mitt", "Romney", partyRepository.findById(parties.get(1).getId()).get(),
                    geographyRepository.findById(geographies.get(0).getId()).get()));
            politicians.add(new Politician("Greg", "Abbott", partyRepository.findById(parties.get(1).getId()).get(),
                    geographyRepository.findById(geographies.get(1).getId()).get()));
            politicians.add(new Politician("Beto", "O'Rourke", partyRepository.findById(parties.get(0).getId()).get(),
                    geographyRepository.findById(geographies.get(1).getId()).get()));
            politicians.add(new Politician("Foo", "Person", partyRepository.findById(parties.get(0).getId()).get(),
                    geographyRepository.findById(geographies.get(2).getId()).get()));
            politicians.add(new Politician("Bar", "Person", partyRepository.findById(parties.get(1).getId()).get(),
                    geographyRepository.findById(geographies.get(2).getId()).get()));
            politicians.add(new Politician("Fizz", "Politician", partyRepository.findById(parties.get(0).getId()).get(),
                    geographyRepository.findById(geographies.get(3).getId()).get()));
            politicians.add(new Politician("Buzz", "Politician", partyRepository.findById(parties.get(1).getId()).get(),
                    geographyRepository.findById(geographies.get(3).getId()).get()));
